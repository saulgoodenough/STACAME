...
def Cal_Spatial_Net(adata, rad_cutoff=None, k_cutoff=None,
                    max_neigh=50, model='Radius', verbose=True, use_key='spatial'):
    """\
    ...
    model
        The network construction model. When model=='Radius', the spot is connected to spots 
        whose distance is less than rad_cutoff. When model=='KNN', the spot is connected to 
        its first k_cutoff nearest neighbors.
    use_key
        The key in adata.obsm to use for coordinates. When use_key='spatial', uses Euclidean 
        distance. Otherwise, uses cosine distance.
    Returns
    -------
    ...
    coor = pd.DataFrame(adata.obsm[use_key])
    dim_spatial = adata.obsm[use_key].shape[1]
    
    coor.index = adata.obs.index
    if dim_spatial == 2:
        coor.columns = ['imagerow', 'imagecol']
    elif dim_spatial == 3:
        coor.columns = ['imagerow', 'imagecol', 'imagehigh']
    else:
        coor.columns = ['imagecol'+str(int(i)) for i in range(dim_spatial)]
    
    # Choose distance metric and algorithm based on use_key
    if use_key == 'spatial':
        # Use Euclidean distance for spatial coordinates
        metric = 'euclidean'
        algorithm = 'ball_tree'
        if verbose:
            print('Using Euclidean distance for spatial coordinates')
    else:
        # Use cosine distance for other embeddings (e.g., feature embeddings)
        metric = 'cosine'
        algorithm = 'brute'  # ball_tree doesn't support cosine distance
        if verbose:
            print(f'Using cosine distance for {use_key} embeddings')
    
    # Create NearestNeighbors object with appropriate metric
    nbrs = sklearn.neighbors.NearestNeighbors(
        n_neighbors=max_neigh + 1, 
        algorithm=algorithm,
        metric=metric
    ).fit(coor)
    ...
    # Optional: add self loops (currently commented out)
    # self_loops = pd.DataFrame(zip(Spatial_Net['Cell1'].unique(), Spatial_Net['Cell1'].unique(),
    #                  [0] * len((Spatial_Net['Cell1'].unique()))))
...
def Cal_SpatialExpression_Net(adata, rad_cutoff=None, rad_cutoff_express=None, k_cutoff=None,
                    max_neigh=50, model='Radius', verbose=True, use_key='spatial', use_express_key='spatial_pca'):
    """\
    Construct the spatial neighbor networks based on intersection of spatial and expression graphs.
    Parameters
    ----------
    adata
        AnnData object of scanpy package.
    rad_cutoff
        radius cutoff when model='Radius' for spatial coordinates
    rad_cutoff_express
        radius cutoff when model='Radius' for expression data
    k_cutoff
        The number of nearest neighbors when model='KNN'
    model
        The network construction model. When model=='Radius', the spot is connected to spots 
        ...
        its first k_cutoff nearest neighbors.
    use_key
        The key in adata.obsm to use for spatial coordinates.
    use_express_key
        The key in adata.obsm to use for expression coordinates.
    Returns
    -------
    The spatial networks are saved in adata.uns['Spatial_Net']
    """
    ...
    # Build spatial graph
    coor_spatial = pd.DataFrame(adata.obsm[use_key])
    nbrs_spatial = sklearn.neighbors.NearestNeighbors(
        n_neighbors=max_neigh + 1, 
        algorithm='ball_tree',
        metric='euclidean'
    ).fit(coor_spatial)
    
    distances_spatial, indices_spatial = nbrs_spatial.kneighbors(coor_spatial)
    
    if model == 'KNN':
        indices_spatial = indices_spatial[:, 1:k_cutoff + 1]
        distances_spatial = distances_spatial[:, 1:k_cutoff + 1]
    if model == 'Radius':
        indices_spatial = indices_spatial[:, 1:]
        distances_spatial = distances_spatial[:, 1:]
    
    KNN_list_spatial = []
    for it in range(indices_spatial.shape[0]):
        KNN_list_spatial.append(pd.DataFrame(zip([it] * indices_spatial.shape[1], indices_spatial[it, :], distances_spatial[it, :])))
    
    KNN_df_spatial = pd.concat(KNN_list_spatial)
    KNN_df_spatial.columns = ['Cell1', 'Cell2', 'Distance']
    
    Spatial_Net_spatial = KNN_df_spatial.copy()
    if model == 'Radius':
        Spatial_Net_spatial = KNN_df_spatial.loc[KNN_df_spatial['Distance'] < rad_cutoff,]
    
    # Build expression graph
    coor_express = pd.DataFrame(adata.obsm[use_express_key])
    nbrs_express = sklearn.neighbors.NearestNeighbors(
        n_neighbors=max_neigh + 1, 
        algorithm='brute',
        metric='cosine'
    ).fit(coor_express)
    
    distances_express, indices_express = nbrs_express.kneighbors(coor_express)
    
    if model == 'KNN':
        indices_express = indices_express[:, 1:k_cutoff + 1]
        distances_express = distances_express[:, 1:k_cutoff + 1]
    if model == 'Radius':
        indices_express = indices_express[:, 1:]
        distances_express = distances_express[:, 1:]
        if rad_cutoff_express is None:
            rad_cutoff_express = rad_cutoff
    
    KNN_list_express = []
    for it in range(indices_express.shape[0]):
        KNN_list_express.append(pd.DataFrame(zip([it] * indices_express.shape[1], indices_express[it, :], distances_express[it, :])))
    
    KNN_df_express = pd.concat(KNN_list_express)
    KNN_df_express.columns = ['Cell1', 'Cell2', 'Distance']
    
    Spatial_Net_express = KNN_df_express.copy()
    if model == 'Radius':
        Spatial_Net_express = KNN_df_express.loc[KNN_df_express['Distance'] < rad_cutoff_express,]
    
    # Map indices to cell names
    id_cell_trans = dict(zip(range(coor_spatial.shape[0]), np.array(adata.obs.index)))
    Spatial_Net_spatial['Cell1'] = Spatial_Net_spatial['Cell1'].map(id_cell_trans)
    Spatial_Net_spatial['Cell2'] = Spatial_Net_spatial['Cell2'].map(id_cell_trans)
    Spatial_Net_express['Cell1'] = Spatial_Net_express['Cell1'].map(id_cell_trans)
    Spatial_Net_express['Cell2'] = Spatial_Net_express['Cell2'].map(id_cell_trans)
    
    # Print statistics for both graphs
    if verbose:
        print(f'Spatial graph contains {Spatial_Net_spatial.shape[0]} edges, {adata.n_obs} cells.')
        print(f'Expression graph contains {Spatial_Net_express.shape[0]} edges, {adata.n_obs} cells.')
        print(f'Spatial graph: {Spatial_Net_spatial.shape[0] / adata.n_obs:.4f} neighbors per cell on average.')
        print(f'Expression graph: {Spatial_Net_express.shape[0] / adata.n_obs:.4f} neighbors per cell on average.')
    
    # Find intersection of both graphs
    Spatial_Net_combined = pd.merge(Spatial_Net_spatial, Spatial_Net_express, 
                                   on=['Cell1', 'Cell2'], how='inner')
    Spatial_Net = Spatial_Net_combined[['Cell1', 'Cell2', 'Distance_x']].copy()
    Spatial_Net.columns = ['Cell1', 'Cell2', 'Distance']
    
    if verbose:
        print(f'Intersection graph contains {Spatial_Net.shape[0]} edges, {adata.n_obs} cells.')
        print(f'Intersection graph: {Spatial_Net.shape[0] / adata.n_obs:.4f} neighbors per cell on average.')
        print(f'Intersection ratio: {Spatial_Net.shape[0] / min(Spatial_Net_spatial.shape[0], Spatial_Net_express.shape[0]):.4f}')
...
def ICP_align(adata_concat, adata_target, adata_ref, slice_target, slice_ref, landmark_domain, plot_align=False):
    ...
    mnn_dict = create_dictionary_mnn(batch_pair, use_rep='STACAME', batch_name='batch_name', k=1, iter_comb=None, verbose=0)
    ...
    anchor_arr = adata_concat.obsm['STACAME'][anchor_ind, ]
    positive_arr = adata_concat.obsm['STACAME'][positive_ind, ]
